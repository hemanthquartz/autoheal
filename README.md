...
def lambda_handler(event, context):
    ...
    bucket = (event.get("evidenceBucket")
              or (event.get("detail") or {}).get("evidenceBucket")
              or __import__("os").environ.get("AUTOSYS_EVIDENCE_BUCKET"))

    prefix = (event.get("evidencePrefix")
              or (event.get("detail") or {}).get("evidencePrefix")
              or __import__("os").environ.get("AUTOSYS_EVIDENCE_PREFIX", "autosys-evidence"))
    ...
    instance_id = event.get("instanceId") or detail.get("instanceId")
    job_name = event.get("jobName") or detail.get("jobName")
    run_as_user = event.get("runAsUser") or detail.get("runAsUser") or "ec2-user"
    ...
    # Runs on EC2: collect BEFORE, run sendevent, collect AFTER, save to LOCAL FILE, upload file to S3
    remote_script = f"""
set -e
